...
from ...config_ocean_model import OceanModelMethods
...
"""
- What/how will this be used
    1. UI needs to know how to configure a simulation
        a. Make api request
        b. api: OpenDriftConfig.model_json_schema
    2. UI configures an OpenDriftConfig, sends it back as json
    3. PTT API nowo has a valid 
    4. Now start a simulation:
        OpenDriftModel.from_config(opendrift_conf_instance) --> Opendriftmodelinstance
        modelinstance.seed, modelinstance



- How to connect 1st class config to rest of implementation


- Decorators/computer fields + properties
"""
...
#     # @classmethod
#     # def from_config(cls, model_config: OpenDriftConfig) --> Self:
#     #     return OpenDriftModel(...)

#     #     # OpenDriftModel(lat="", lon="")
...
# class OpenDriftModel(ParticleTrackingManager[OpenDriftConfig]):
class OpenDriftModel(ParticleTrackingManager):
    """OpenDrift particle tracking model."""

    # def __init__(self, config: OpenDriftConfig):
    ...
    def __init__(self, **kwargs):
        ...
        # ptm.OpenDriftConfig(drift_model={"drift_model": "OpenOil", "steps": 1}).drift_model.model_fields["current_uncertainty"] #.current_uncertainty
        ...
        # inputs = {key: getattr(self.config.drift_model,key) for key in keys_from_the_manager}
        ...
        self.config = OpenDriftConfig(drift_model={**kwargs})
        # self.config = OpenDriftConfig(drift_model={"drift_model": "OpenOil", **kwargs})
        # self.config = OpenDriftConfig(drift_model={"drift_model": kwargs["drift_model"]})
        # self.config = OpenDriftConfig(kwargs)  # this runs both OpenDriftConfig and TheManagerConfig
        # logger = self.config.logger  # this is where logger is expected to be found
        # import pdb; pdb.set_trace()
        
        self.ocean_model_methods = OceanModelMethods(ocean_model=self.ocean_model)  # validates ocean model for simulation and has useful functions
    ...
    @classmethod
    def from_config(cls, config: OpenDriftConfig) -> Self:
        """Create an OpenDriftModel from a config."""
        return cls(**config.drift_model.dict())
    ...
    def _create_opendrift_model_object(self):
        ...
        if self.config.drift_model.drift_model == "Leeway":
        ...
        elif self.config.drift_model.drift_model == "OceanDrift":
            from opendrift.models.oceandrift import OceanDrift
            o = OceanDrift(
                loglevel=log_level,
            )  # , output_format=self.output_format)

        elif self.config.drift_model.drift_model == "LarvalFish":
            from opendrift.models.larvalfish import LarvalFish
            o = LarvalFish(
                loglevel=log_level
            )  # , output_format=self.output_format)

        elif self.config.drift_model.drift_model == "OpenOil":
            ...
            raise ValueError(f"Drifter model {self.config.drift_model.drift_model} is not recognized.")
    ...
    def _update_od_config_from_this_config(self):
        ...
        base_models_to_check = [self.config.drift_model, self.files, self.ocean_model]
        # base_models_to_check = [self.files, self.config.drift_model, self.ocean_model, self.config]
    ...
    def _modify_opendrift_model_object(self):
        ...
        # import pdb; pdb.set_trace()
        if self.config.drift_model.stokes_drift:
        ...
        if not self.config.drift_model.do3D and self.config.drift_model.z == 0 and self.config.drift_model.drift_model != "Leeway":
        ...
        if not self.config.drift_model.do3D and self.config.drift_model.drift_model != "Leeway":
        ...
        if self.config.drift_model.do3D:
    ...
    def add_reader(
        ...
        if (
            ...
            and self.ocean_model.name != "test"
            and ds is None
        ):
            raise ValueError(
        ...
        if ds is not None:
            if name is None:
                self.ocean_model.name = "user_input"
            else:
                self.ocean_model.name = name

        # TODO: do I still need a pathway for ocean_model of "test"?
        # TODO: move tests from test_manager to other files
        else:
            ds = self.ocean_model_methods.open_dataset(drop_vars=[], ocean_model_local=self.config.drift_model.ocean_model_local)
            # TODO: reinstate the following line
            # ds = self.ocean_model_methods.open_dataset(drop_vars=self.config.drift_model.drop_vars)
        ...
        if not self.config.drift_model.ocean_model_local:
            ds = narrow_dataset_to_simulation_time(ds, self.config.drift_model.start_time, self.config.drift_model.end_time)
            logger.info("Narrowed model output to simulation time")
        
        ds = apply_known_ocean_model_specific_changes(ds, self.ocean_model.name, self.config.drift_model.use_static_masks)
        
        # TODO: the stuff in apply_user_input_ocean_model_specific_changes can be moved to OceanModelConfig
        # validation I think
        if self.ocean_model.name not in _KNOWN_MODELS and self.ocean_model.name != "test":
            ds = apply_user_input_ocean_model_specific_changes(ds, self.config.drift_model.use_static_mask)

        self.ds = ds

        # if self.ocean_model.name == "test":
        #     pass
        #     # oceanmodel_lon0_360 = True
        #     # loc = "test"
        #     # kwargs_xarray = dict()

        # elif self.ocean_model.name is not None or ds is not None:
        #     # pass
        ...
        #     # if not self.config.drift_model.do3D:
        #     #     drop_vars += ["w"]
        ...
        #     # if (
        #     #     not self.config.drift_model.stokes_drift
        #     #     and self.config.drift_model.wind_drift_factor == 0
        #     #     and self.config.drift_model.wind_uncertainty == 0
        #     #     and self.config.drift_model.drift_model != "OpenOil"
        #     #     and not self.config.drift_model.vertical_mixing
        #     # ):
        ...
        #     # if self.config.drift_model.drift_model not in ["LarvalFish", "OpenOil"]:
        #     #     drop_vars += ["salt", "temp"]
        #     #     logger.info(
        ...
        #     # if self.config.drift_model.drift_model != "OpenOil":
        ...
        #     # if self.config.drift_model.use_static_masks:
        ...
        #     # if self.ocean_model.name == "NWGOA":
        #     #     oceanmodel_lon0_360 = True
        ...
        #     # elif "CIOFS" in self.ocean_model.name:
        #     #     oceanmodel_lon0_360 = False

        #     #     drop_vars += [
        #     #         "wetdry_mask_psi",
        #     #     ]
        #     #     if self.ocean_model.name == "CIOFS":
        ...
        #     #                     "Need to set start_time ahead of time to add local reader."
        #     #                 )
        #     #             start = f"{self.config.drift_model.start_time.year}_{str(self.config.drift_model.start_time.dayofyear - 1).zfill(4)}"
        #     #             end = f"{self.config.drift_model.end_time.year}_{str(self.config.drift_model.end_time.dayofyear).zfill(4)}"
        ...
        #     #     elif self.ocean_model.name == "CIOFSFRESH":
        ...
        #     #     elif self.ocean_model.name == "CIOFSOP":
        ...
        #     #                     "Need to set start_time ahead of time to add local reader."
        #     #                 )
        #     #             start = f"{self.config.drift_model.start_time.year}-{str(self.config.drift_model.start_time.month).zfill(2)}-{str(self.config.drift_model.start_time.day).zfill(2)}"
        #     #             end = f"{self.config.drift_model.end_time.year}-{str(self.config.drift_model.end_time.month).zfill(2)}-{str(self.config.drift_model.end_time.day).zfill(2)}"
        ...
        #     # if self.ocean_model.name == "user_input":

        #     #     # check for case that self.config.drift_model.use_static_masks False (which is the default)
        #     #     # but user input doesn't have wetdry masks
        #     #     # then raise exception and tell user to set use_static_masks True
        #     #     if "wetdry_mask_rho" not in ds.data_vars and not self.config.drift_model.use_static_masks:
        #     #         raise ValueError(
        #     #             "User input does not have wetdry_mask_rho variable. Set use_static_masks True to use static masks instead."
        #     #         )

        #     #     ds = ds.drop_vars(self.config.drift_model.drop_vars, errors="ignore")
        ...
        #     #         ds = xr.open_dataset(
        #     #             self.config.drift_model.loc_local,
        #     #             engine="kerchunk",
        #     #             # chunks={},  # Looks like it is faster not to include this for kerchunk
        #     #             drop_variables=self.config.drift_model.drop_vars,
        #     #             decode_times=False,
        #     #         )

        #     #         logger.info(
        #     #             f"Opened local dataset starting {self.config.drift_model.start_time} and ending {self.config.drift_model.end_time} with number outputs {ds.ocean_time.size}."
        #     #         )

        #     #     # otherwise remote
        #     #     else:
        #     #         if ".nc" in self.config.drift_model.loc_remote:

        #     #             if self.ocean_model.name == "CIOFSFRESH":
        #     #                 raise NotImplementedError

        #     #             ds = xr.open_dataset(
        #     #                 self.config.drift_model.loc_remote,
        #     #                 chunks={},
        #     #                 drop_variables=self.config.drift_model.drop_vars,
        #     #                 decode_times=False,
        #     #             )
        ...
        #     #         logger.info(
        #     #             f"Opened remote dataset {self.config.drift_model.loc_remote} with number outputs {ds.ocean_time.size}."
        #     #         )

        #     # # For NWGOA, need to calculate wetdry mask from a variable
        #     # if self.ocean_model.name == "NWGOA" and not self.config.drift_model.use_static_masks:
        ...
        #     #     # self.config.drift_model.start_time = reader.start_time
        #     #     # convert using pandas instead of netCDF4
        #     #     self.config.drift_model.start_time = units_date + pd.to_timedelta(
        #     #         ds.ocean_time[0].values, unit="s"
        #     #     )
        #     # # narrow model output to simulation time if possible before sending to Reader
        #     # if self.config.drift_model.start_time is not None and self.config.drift_model.end_time is not None:
        #     #     dt_model = float(
        ...
        #     #     start_time_num = (
        #     #         self.config.drift_model.start_time - units_date
        #     #     ).total_seconds() - dt_model
        #     #     # want to include the next ocean model output after the last drifter simulation time
        #     #     end_time_num = (self.config.drift_model.end_time - units_date).total_seconds() + dt_model
        ...
        reader = reader_ROMS_native.Reader(
            filename=ds,
            name=self.ocean_model.name,
            standard_name_mapping=self.ocean_model.standard_name_mapping,
            save_interpolator=self.config.drift_model.save_interpolator,
            interpolator_filename=self.config.drift_model.interpolator_filename,
        )
    ...
    def seed_kws(self):
        ...
        if self.config.drift_model.start_time_end is not None:
            # time can be a list to start drifters linearly in time
            time = [
                self.config.drift_model.start_time.to_pydatetime(),
                self.config.drift_model.start_time_end.to_pydatetime(),
            ]
        elif self.config.drift_model.start_time is not None:
            time = self.config.drift_model.start_time
            # time = self.config.drift_model.start_time.to_pydatetime()
        else:
            time = None

        _seed_kws = {
            "time": time,
            "z": self.config.drift_model.z,
        }
        ...
        if self.config.drift_model.seed_flag == "elements":
            # add additional seed parameters
            _seed_kws.update(
                {
                    "lon": self.config.drift_model.lon,
                    "lat": self.config.drift_model.lat,
                    "radius": self.config.drift_model.radius,
                    "radius_type": self.config.drift_model.radius_type,
                }
            )
            ...
            _seed_kws["time"] = (
                self.config.drift_model.start_time.isoformat() if self.config.drift_model.start_time is not None else None
            )
    ...
    def seed(self):
        ...
        elif self.config.drift_model.seed_flag == "geojson":

            # # geojson needs string representation of time
            # self.seed_kws["time"] = self.config.drift_model.start_time.isoformat()
            self.geojson["properties"] = self.seed_kws
            json_string_dumps = json.dumps(self.geojson)
            self.o.seed_from_geojson(json_string_dumps)

        else:
            raise ValueError(f"seed_flag {self.config.drift_model.seed_flag} not recognized.")
    ...
    def run(self):
        ...
        logger.info(f"start_time: {self.config.drift_model.start_time}, end_time: {self.config.drift_model.end_time}, steps: {self.config.drift_model.steps}, duration: {self.config.drift_model.duration}")
        ...
        self.o.metadata_dict.update(self.config.drift_model.dict())
        self.o.metadata_dict.update(self.config.drift_model.dict())
        ...
        # actually run
        self.o.run(
            time_step=self.config.drift_model.time_step,
            time_step_output=self.config.drift_model.time_step_output,
            steps=self.config.drift_model.steps,
            export_variables=self.config.drift_model.export_variables,
            outfile=self.files.output_file,
        )

        # plot if requested
        if self.config.drift_model.plots:
            # TODO: fix this for Ahmad
            # return plots because now contains the filenames for each plot
            self.config.drift_model.plots = make_plots(
                self.config.drift_model.plots, self.o, self.files.output_file.split(".")[0], self.config.drift_model.drift_model
            )
            ...
            self.config.drift_model.plots = repr(self.config.drift_model.plots)
    ...
    def all_config(self):
        ...
        if self._all_config is None:
            self._all_config = {**self._model_config, **self.config.drift_model.dict()}
    ...
    def reader_metadata(self, key):
        ...
        return self.o.env.readers[self.ocean_model.name].__dict__[key]
