...
class OpenDriftModel(ParticleTrackingManager):
    ...
    def __init__(self, **kwargs):
        ...
        self.ocean_model_methods = OceanModelMethods(ocean_model=self.ocean_model, config=self.config)  # validates ocean model for simulation and has useful functions
    ...
    def _update_od_config_from_this_config(self):
        ...
        base_models_to_check = [self.config, self.files, self.ocean_model]
        # base_models_to_check = [self.files, self.config, self.ocean_model, self.config]
    ...
    def add_reader(
        ...
        # TODO: move tests from test_manager to other files
            ...
            ds = self.ocean_model_methods.open_dataset(drop_vars=[])
            # TODO: reinstate the following line
            # ds = self.ocean_model_methods.open_dataset(drop_vars=self.config.drop_vars)
        ...
        ds = apply_known_ocean_model_specific_changes(ds, self.ocean_model.name, self.config.use_static_masks)
        ...
        if self.ocean_model.name not in _KNOWN_MODELS and self.ocean_model.name != "test":
            ds = apply_user_input_ocean_model_specific_changes(ds, self.config.use_static_mask)
        ...
        #     # if self.ocean_model.name == "NWGOA" and not self.config.use_static_masks:
