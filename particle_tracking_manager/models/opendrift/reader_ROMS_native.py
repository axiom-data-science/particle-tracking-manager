...
from opendrift.readers.basereader import BaseReader, StructuredReader, vector_pairs_xy
from opendrift.readers.roppy import depth


class Reader(BaseReader, StructuredReader):
    """Reader for ROMS models."""
    ...
    def __init__(
            ...
            if ("*" in filestr) or ("?" in filestr) or ("[" in filestr):
                ...
                def drop_non_essential_vars_pop(ds):
                    ...
                    dropvars = [
                        ...
                        if v
                        not in list(self.ROMS_variable_mapping.keys())
                        + gls_param
                        + [
                            "ocean_time",
                            "time",
                ...
                self.Dataset = xr.open_dataset(
                    filename, decode_times=False, **kwargs_xarray
                )
        ...
        if dimensions == 3:
            ...
            try:
                ...
                self.sigma = (np.arange(num_sigma) + 0.5 - num_sigma) / num_sigma
        ...
        if "lat_rho" in self.Dataset.variables:
            ...
            raise ValueError(
                filename + " does not contain lon/lat "
        ...
        try:  # Check for GLS parameters (diffusivity)
            ...
            for gls_par in gls_param:
                self.gls_parameters[gls_par] = self.Dataset.variables[gls_par][()]
        ...
        if time_units == "second":
            logger.info(
                ...
                "Setting artificial start time of 1 Jan 2000."
            )
            time_units = "seconds since 2000-01-01 00:00:00"
        if time_units == "days":
            logger.info(
                "Ocean time given as days relative to start "
                "Setting artificial start time of 1 Jan 2000."
            )
        ...
        for var_name in list(self.Dataset.variables):
            ...
            if (
                "standard_name" in var.attrs
                and var_name not in self.ROMS_variable_mapping.keys()
            ):
        ...
        for var, stdname in self.ROMS_variable_mapping.items():
            if (
                "east" in var.lower()
                or "east" in stdname.lower()
                or "north" in var.lower()
                or "north" in stdname.lower()
            ):
                self.do_not_rotate.append(stdname)
        if len(self.do_not_rotate) > 0:
            logger.debug(
                "The following ROMS vectors are considered east-north, and will not be rotated %s"
                % self.do_not_rotate
            )
    ...
    def get_variables(self, requested_variables, time=None, x=None, y=None, z=None):
        """Get variables from model output files."""
        ...
        if z.min() == 0 or not hasattr(self, "hc"):
            ...
            if not hasattr(self, "z_rho_tot"):
                ...
                self.z_rho_tot = depth.sdepth(
                    Htot, self.hc, self.Cs_r, Vtransform=self.Vtransform
                )

            H = self.sea_floor_depth_below_sea_level[indy, indx]
            z_rho = depth.sdepth(H, self.hc, self.Cs_r, Vtransform=self.Vtransform)
            ...
            indz = range(
                ...
                np.minimum(self.num_layers, indz_max + 1 + self.verticalbuffer),
            )
            z_rho = z_rho[indz, :, :]
            # Determine the z-levels to which to interpolate
            zi1 = np.maximum(
                0, bisect_left(-np.array(self.zlevels), -z.max()) - self.verticalbuffer
            )
            zi2 = np.minimum(
                len(self.zlevels),
                bisect_right(-np.array(self.zlevels), -z.min()) + self.verticalbuffer,
            )
        ...
        for par in requested_variables:
            varname = [
                name for name, cf in self.ROMS_variable_mapping.items() if cf == par
            ]
            ...
            if par == "land_binary_mask":
                if var.ndim == 2:
                    ...
                    variables[par] = self.Dataset["wetdry_mask_rho"][
                        indxTime, indy, indx
                    ]
                ...
                raise Exception(
                    "Wrong dimension of variable: " + self.variable_mapping[par]
                )
                ...
                if par in [
                    ...
                    if not hasattr(self, "land_binary_mask"):
                        ...
                        if "mask_rho" in self.Dataset.variables:
                            ...
                            self.land_binary_mask = (
                                1 - self.Dataset.variables["mask_psi"][:]
                            )
            ...
            if var.ndim == 4:
                ...
                if len(np.atleast_1d(indz)) > 1:
                    ...
                    variables[par][variables[par] > 1e9] = np.nan
        ...
        if (
            "x_sea_water_velocity" in variables.keys()
            or "sea_ice_x_velocity" in variables.keys()
            or "x_wind" in variables.keys()
            and "x_sea_water_velocity" not in self.do_not_rotate
            and "sea_ice_x_velocity" not in self.do_not_rotate
            and "x_wind" not in self.do_not_rotate
        ):
            ...
            if (
                "sea_ice_x_velocity" in variables.keys()
                and "sea_ice_x_velocity" not in self.do_not_rotate
            ):
                (
                    variables["sea_ice_x_velocity"],
                    variables["sea_ice_y_velocity"],
                ) = rotate_vectors_angle(
                    variables["sea_ice_x_velocity"],
                    variables["sea_ice_y_velocity"],
                    rad,
            ...
            if "x_wind" in variables.keys() and "x_wind" not in self.do_not_rotate:
                variables["x_wind"], variables["y_wind"] = rotate_vectors_angle(
                    variables["x_wind"], variables["y_wind"], rad
                )
...
def rotate_vectors_angle(u, v, radians):
    """Rotate vectors to be east/north."""
