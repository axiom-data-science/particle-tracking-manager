...
tests_invalid = {"NWGOA": {"lon": 185-360, "lat": 50, "start_time": "2022-01-01", "end_time": "2022-01-01T05:00", "ocean_model_config": ocean_model_registry.get("NWGOA")},
            "CIOFS": {"lon": -145, "lat": 40, "start_time": "2024-01-01", "end_time": "2024-01-01T05:00", "ocean_model_config": ocean_model_registry.get("CIOFS")},
            "CIOFSOP": {"lon": -145, "lat": 40, "start_time": "2020-01-01", "end_time": "2020-01-01T05:00", "ocean_model_config": ocean_model_registry.get("CIOFSOP")},
            "CIOFSFRESH": {"lon": -145, "lat": 40, "start_time": "2022-01-01", "end_time": "2022-01-01T05:00", "ocean_model_config": ocean_model_registry.get("CIOFSFRESH")}}
...
def test_lon_lat():
    ...
    for ocean_model, test in tests_valid.items():
        m = ocean_model_simulation_mapper[ocean_model](start_time=test["start_time"], end_time=test["end_time"],
                                    lon=test["lon"], lat=test["lat"], ocean_model_local=True)

    for ocean_model, test in tests_invalid.items():
        with pytest.raises(ValidationError):
            m = ocean_model_simulation_mapper[ocean_model](start_time=tests_valid[ocean_model]["start_time"], end_time=tests_valid[ocean_model]["end_time"],
...
def test_oceanmodel_lon0_360():
    ...
    m = ocean_model_simulation_mapper["CIOFSOP"](start_time="2022-01-01", lon=lon_in, lat=57, end_time="2022-01-01T05:00", 
                                    ocean_model_local=True)
    assert m.ocean_model_config.oceanmodel_lon0_360 == False
    assert m.lon == lon_in

    m = ocean_model_simulation_mapper["CIOFS"](start_time="2022-01-01", lon=lon_in, lat=57, end_time="2022-01-01T05:00", 
                                    ocean_model_local=True)
    assert m.ocean_model_config.oceanmodel_lon0_360 == False
    assert m.lon == lon_in

    m = ocean_model_simulation_mapper["CIOFSFRESH"](start_time="2004-01-01", lon=lon_in, lat=57, end_time="2004-01-01T05:00", 
                                    ocean_model_local=True)
    assert m.ocean_model_config.oceanmodel_lon0_360 == False
    assert m.lon == lon_in
    
    m = ocean_model_simulation_mapper["NWGOA"](start_time="2007-01-01", lon=lon_in, lat=57, end_time="2007-01-01T05:00",
                                    ocean_model_local=True)
...
def test_start_end_times():
    ...
    for ocean_model, test in tests_invalid.items():
        with pytest.raises(ValidationError):
            m = ocean_model_simulation_mapper[ocean_model](start_time=test["start_time"], end_time=tests_valid[ocean_model]["end_time"],
                                        lon=tests_valid[ocean_model]["lon"], lat=tests_valid[ocean_model]["lat"], ocean_model_local=True)
        with pytest.raises(ValidationError):
            m = ocean_model_simulation_mapper[ocean_model](start_time=tests_valid[ocean_model]["start_time"], end_time=test["end_time"],
                                        lon=tests_valid[ocean_model]["lon"], lat=tests_valid[ocean_model]["lat"], ocean_model_local=True)
